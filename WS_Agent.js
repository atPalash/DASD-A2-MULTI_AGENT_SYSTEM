var WS_Status = ['WS','WS1','WS2','WS3','WS4','WS5','WS6','WS7','WS8','WS9','WS10','WS11','WS12'];
...
WS_Agent.prototype.runServer = function () {
    ...
    var stat = "busy";
    function neighbourStat(url){
        request.get(url, function (err, res, body) {
            stat = body;
        });
        if(stat == "free"){
            getNeighbourStat(stat);
        }
    }
    function getNeighbourStat(stat) {
        return stat;
    }
    ...
    app.post('/'+WS+'notifs', function (req, res) {
        ...
        switch (event) {
            case "Z1_Changed": {
                setTimeout(function () {
                    if((WS_Status[WSnum] != "busy")||(WS_ID=='WS1')){
                        if (req.body.payload.PalletID != -1) {
                            WS_Status[WSnum] = "busy";
                            console.log(WS_ID, WS_Status[WSnum]);
                            var currentPallet = getPallet();
                            var port = currentPallet.port_;
            ...
            case "Z4_Changed": {
                ...
                if (req.body.payload.PalletID != -1) {
                    if(WSnum == 12){
                        while(WS_Status[1] == "busy"){
                            counter++;
                            console.log("hi");
                        }
                    }else{
                        while(WS_Status[WSnum+1] == "busy"){
                            console.log("hi");
                            counter++;
                        }
                    }
            ...
            case "PaperLoaded":{
                ...
                var currentPallet = getPallet();
                var port = currentPallet.port_;
                var palletID = currentPallet.palletID_;
